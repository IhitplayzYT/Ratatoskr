...
pub struct Note_task {
    ...
    impl Note_task{
        pub fn new(title:Option<String>,content: Option<String>,pinned: bool,favorite:bool,tags:Option<Vec<Tag>>,part_of: Option<String>,topic:Option<String>) -> Note_task{
            Self { id:Uuid::new_v4(), title:if let Some(x) = title{x} else {"Untitled".to_string()}, content: if let Some(x) = content{x} else {"".to_string()},created_at: Utc::now(), updated_at: Utc::now(),tags: if let Some(x) = tags {x} else{Vec::new()},member:part_of,topic,pinned,favorite}
        }
