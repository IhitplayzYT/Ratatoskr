pub mod Pomodero{
    ...
    impl Default for Pomodero{
        fn default() -> Self {
            Self { duration_left:600,duration: 600, fmt: "{hh}:{mm}:{ss}".to_string(),running:false,paused:false,focus:PomoFocus::Hour}
        }
    }
    ...
    pub fn new(duration:usize,fmt:Option<String>,color:Option<MyColor>) ->Self{
        Self { duration,duration_left:duration, fmt:fmt.unwrap_or("{hh}:{mm}:{ss}".to_string()),running:false,paused:false,focus:PomoFocus::Hour}
    }
