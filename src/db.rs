...
use crate::model::app::App::Feature_set;
...
tx.query_drop("
    ...
    tx.query_drop("
    CREATE TABLE IF NOT EXISTS Journal_task_tags(
        task_id CHAR(36),
        tag_id INT,
...
tx.query_drop("
    ...
    CREATE TABLE IF NOT EXISTS Note_task_tags(
        task_id CHAR(36),
        tag_id INT,
        PRIMARY KEY(task_id,tag_id),
...
pub fn update_event(&self,id:Uuid,cal:&Calendar_task) -> mysql::Result<()>{
    let mut conn = self.conn()?;
    let mut tx = conn.start_transaction(TxOpts::default())?;
    tx.exec_drop("UPDATE Calendar SET event=?,description=?,duration=?,frequency=?,event_date=?,color=? WHERE id=?;", (&cal.event,&cal.desc,String::from(cal.duration),String::from(cal.frequency),cal.date.naive_utc(),cal.color.rgb_str(),id.to_string()))?;
